/*
 * Complete Tm_NN Implementation in C with ALL BioPython tables
 * Supports DNA_NN1, DNA_NN2, DNA_NN3, DNA_NN4, RNA_NN1, RNA_NN2, RNA_NN3, R_DNA_NN1
 */
...
#include "thermodynamic_tables.h"
...
// Function prototypes
double tm_nn_table(const char* seq, const char* c_seq, int shift,
                   double dnac1, double dnac2, bool selfcomp,
                   double Na, double K, double Tris, double Mg, double dNTPs,
                   int saltcorr, int table_id);
double salt_correction(double Na, double K, double Tris, double Mg, double dNTPs,
                      int method, const char* seq, int seq_len);
double gc_fraction(const char* seq, int len);
int find_therm_params(const TableEntry* table, const char* key, ThermParams* params);
void reverse_string(char* str);
char* get_complement(const char* seq);
void clean_sequence(const char* input, char* output);
void uppercase_string(char* str);
const TableEntry* get_nn_table(int table_id);
const TableEntry* get_imm_table(int table_id);
const TableEntry* get_tmm_table(int table_id);
const TableEntry* get_de_table(int table_id);
...
char g_error_nn_key[MAX_KEY_LENGTH] = {0};
...
// Wrapper for backward compatibility (defaults to DNA_NN4)
double tm_nn(const char* seq, const char* c_seq, int shift,
             double dnac1, double dnac2, bool selfcomp,
             double Na, double K, double Tris, double Mg, double dNTPs,
             int saltcorr) {
    return tm_nn_table(seq, c_seq, shift, dnac1, dnac2, selfcomp,
                      Na, K, Tris, Mg, dNTPs, saltcorr, TABLE_DNA_NN4);
}