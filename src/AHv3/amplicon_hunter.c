...
#define VERSION "2.beta"  // Streaming memory management version
...
// Write a sequence to a batch file (FASTA ONLY)
...
#ifdef __linux__
        // Tell kernel to not keep pages in memory after access
...
// Load 2bit file with minimal memory footprint
TwoBitFile load_2bit_file(const char * restrict filename) {
    TwoBitFile file = {0};
    ...
    // Allocate arrays
    file.headers = (char**)calloc(file.num_sequences, sizeof(char*));
    file.lengths = (uint32_t*)calloc(file.num_sequences, sizeof(uint32_t));
    file.data = (uint8_t**)calloc(file.num_sequences, sizeof(uint8_t*));
    file.packed_lengths = (uint32_t*)calloc(file.num_sequences, sizeof(uint32_t));
    
    // Process each sequence header and data pointer
    for (uint32_t i = 0; i < file.num_sequences; i++) {
        ...
        // Just store pointer to mmap'd data, don't copy
        file.data[i] = (uint8_t*)(data + offset);
        offset += file.packed_lengths[i];
        
        // Advise kernel we're done with this part of the file
#ifdef __linux__
        if (offset > MMAP_CHUNK_SIZE && (offset % MMAP_CHUNK_SIZE) < 4096) {
            size_t release_size = offset - (offset % MMAP_CHUNK_SIZE);
            posix_madvise((void*)data, release_size, POSIX_MADV_DONTNEED);
        }
#endif
    }
...
// Free 2bit file with aggressive memory release
void free_2bit_file(TwoBitFile * restrict file) {
    // First free all allocated memory (headers)
    if (file->num_sequences > 0 && file->headers) {
        for (uint32_t i = 0; i < file->num_sequences; i++) {
            if (file->headers[i]) {
                free(file->headers[i]);
                file->headers[i] = NULL;
    ...
    if (file->data) {
        // Note: data array contains pointers into mmap'd region, just free the array
        free(file->data);
        file->data = NULL;
    }
    if (file->packed_lengths) {
        free(file->packed_lengths);
        file->packed_lengths = NULL;
    }
    
    // Finally unmap the file
    unmap_file(&file->mapped);
    
    // Zero out the structure to prevent any accidental reuse
    memset(file, 0, sizeof(TwoBitFile));
}
...
// Worker thread for amplicon finding with streaming memory management
void* amplicon_worker(void *arg) {
    ...
    // Process each file individually with complete memory release
    for (int idx = data->start_idx; idx < data->end_idx; idx++) {
        // Allocate buffers for this file only
        char *seq_buf = NULL;
        size_t seq_cap = 0;
        ...
        for (uint32_t i = 0; i < seq_file.num_sequences; i++) {
            // Reuse buffer for this file
            uint32_t seq_len = seq_file.lengths[i];
            if (seq_len + 1 > seq_cap) {
                seq_cap = seq_len + 1;
                seq_buf = (char*)realloc(seq_buf, seq_cap);
            }
            unpack_sequence(seq_file.data[i], seq_len, seq_buf);
            
            process_sequence_for_amplicons(seq_file.headers[i], seq_buf, 
                                          base_name, data);
            
            // Write amplicons frequently to minimize memory usage
            if (data->num_amplicons > 0 && (i % 100 == 0 || i == seq_file.num_sequences - 1)) {
                write_amplicons_to_temp(data);
            }
        }
        
        // Free sequence buffer for this file
        free(seq_buf);
        seq_buf = NULL;
        seq_cap = 0;
        ...
        // CRITICAL: Free all memory for this file before moving to next
        free_2bit_file(&seq_file);
        
        // Force memory release back to OS after each file
#ifdef __linux__
        malloc_trim(0);
#endif
    }
    
    // Write any remaining amplicons
    if (data->num_amplicons > 0) {
        write_amplicons_to_temp(data);
    }
    
    // Close temp file and ensure everything is written
    if (data->temp_file.fp) {
        fflush(data->temp_file.fp);
...
int run_command(const char *input_list, const char *primers_file, const char *output_file,
    ...
    fprintf(stderr, "Using streaming memory mode with %d-amplicon buffers\n", AMPLICON_BUFFER_SIZE);
...
void print_usage() {
    printf("AmpliconHunter v%s - Streaming memory-efficient amplicon finder\n\n", VERSION);
    printf("Usage:\n");
